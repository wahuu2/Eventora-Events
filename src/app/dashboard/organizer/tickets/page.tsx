      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-14">
            ...
            <div className="relative p-6 sm:p-8 lg:p-10">
              ...
              <div className="relative flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                ...
                <Link
                  ...
                  className="inline-flex min-h-11 w-full shrink-0 items-center justify-center gap-2 rounded-xl border border-border-hover px-5 py-3 text-sm font-semibold transition-all duration-200 hover:border-accent/40 hover:bg-background-secondary sm:w-auto"
                >
        ...
        <section className="mt-8 sm:mt-10">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-7 lg:p-8">
            ...
            <form onSubmit={verifyTicket} className="mt-7">
              <label
              ...
              <div className="mt-2 flex flex-col gap-3 sm:flex-row">
                <div className="relative min-w-0 flex-1">
                  <TicketIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-foreground-muted" />
                  ...
                  <input
                    ...
                    spellCheck={false}
                    disabled={loading}
                    className="min-h-12 w-full rounded-xl border border-border bg-background py-3.5 pl-12 pr-4 font-mono text-sm text-foreground outline-none transition-all duration-200 placeholder:text-foreground-muted focus:border-accent focus:ring-2 focus:ring-accent/20 disabled:cursor-not-allowed disabled:opacity-60"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="inline-flex min-h-12 w-full shrink-0 items-center justify-center gap-2 rounded-xl bg-accent px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-accent/10 transition-all duration-200 hover:bg-accent-hover hover:shadow-accent/20 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                >
                  {loading ? (
                    <>
              ...
              <div className="mt-3 flex items-start gap-2">
                <InformationCircleIcon className="mt-0.5 h-4 w-4 shrink-0 text-foreground-muted" />
                ...
                <div className="p-5 sm:p-6 lg:p-8">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    {(() => {
                      ...
                      return (
                        <span
                          className={`inline-flex w-fit items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold capitalize ${getTicketStatusClasses(
                            ticket.status
                          )}`}
                        >
                  ...
                  {/* Ticket Number */}
                  <div className="mt-6 rounded-2xl border border-border bg-background-secondary p-5 sm:p-6">
                        ...
                        <div className="min-w-0">
                          ...
                          {ticket.user?.email && (
                            <div className="mt-1 flex min-w-0 items-center gap-1.5">
                              <EnvelopeIcon className="h-3.5 w-3.5 shrink-0 text-foreground-muted" />
                    ...
                    <div className="mt-5 rounded-2xl border border-border bg-card p-5 sm:p-6">
                      ...
                      <div className="mt-5 grid gap-4 sm:grid-cols-3">
                        <EventDetail
                          icon={MapPinIcon}
                          label="Location"
                          value={ticket.event.location || "N/A"}
                        />

                        <EventDetail
                          icon={CalendarDaysIcon}
                          label="Date"
                          value={formatDate(ticket.event.date)}
                        />

                        <EventDetail
                          icon={ClockIcon}
                          label="Time"
                          value={ticket.event.time || "N/A"}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {/* Entry Result */}
                  {success && (
                    <div className="mt-6 overflow-hidden rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center sm:p-8">
        ...
        {!message && (
          <section className="mt-6 sm:mt-8">
            <div className="rounded-2xl border border-border bg-background-secondary p-6 shadow-sm sm:p-7">
              <div className="flex items-center gap-2">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                <InstructionStep
                  number="01"
                  title="Get the ticket number"
                  description="Ask the attendee to display their Eventora digital ticket."
                />

                <InstructionStep
                  number="02"
                  title="Verify the ticket"
                  description="Enter the ticket number and submit the verification request."
                />

                <InstructionStep
                  number="03"
                  title="Allow entry"
                  description="Only valid tickets should be accepted at the event entrance."
                />
              </div>
            </div>
          </section>
...
function InstructionStep({
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 transition-colors duration-200 hover:border-border-hover">
      <span className="text-2xl font-bold tracking-tight text-foreground-muted">
        {number}
      </span>
